    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
      <div>
        ...
        <ul className="space-y-2">
          {pros.map((p) => (
            <li key={`pro-${p}`} className="flex items-start gap-2 text-[14px] text-txt/90">
      ...
      <div>
        ...
        <ul className="space-y-2">
          {cons.map((c) => (
            <li key={`con-${c}`} className="flex items-start gap-2 text-[14px] text-txt/90">
...
export function ExchangeLogo({ domain, name, size = 24 }) {
  ...
  if (err || !domain) {
    ...
    return (
      <div
        className="flex items-center justify-center text-[11px] font-mono text-txt/80"
        style={{
          width: size,
          height: size,
