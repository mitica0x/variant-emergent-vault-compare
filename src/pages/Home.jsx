      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 z-20 text-left md:top-[18%] md:right-auto md:left-0 md:p-0 md:pl-20 md:max-w-[600px] md:flex md:flex-col md:gap-8">
        ...
        <div>
          ...
          <motion.p
            ...
            className="mt-6 max-w-xl text-[20px] sm:text-[23px] leading-relaxed"
          ...
          <motion.div
            ...
            className="mt-6 font-mono text-[13px] uppercase tracking-widest"
    ...
    >
      {HERO_METRICS.map((m) => (
        <div key={m.label} className="bg-bg p-4">
          <div className="font-mono text-[28px] text-txt">{m.value}</div>
...
function SurfaceCard({ surface }) {
  ...
  const inner = (
    <>
      <span
        className="absolute top-4 right-4 font-mono text-[11px] uppercase tracking-widest px-2 py-[3px]"
        style={
          lime
      ...
      <p className="mt-3 text-[18px] leading-relaxed" style={{ color: "rgba(255,255,255,0.6)" }}>{desc}</p>
      {!lime && (
        <div className="mt-6 font-mono text-[14px] uppercase tracking-widest" style={{ color: accent }}>{stat}</div>
      )}
    </>
  );
  const cardStyle = {
    ...
    <section className="hairline-t hairline-b">
      <div className="container-x py-24">
        <div className="flex flex-col gap-3 mb-10">
          ...
          <p className="text-[19px] max-w-2xl leading-relaxed" style={{ color: "rgba(255,255,255,0.6)" }}>
        ...
        >
          {OPERATOR_ITEMS.map((s) => (
            <div key={s.value} className="bg-bg p-6">
              <div className="font-mono text-[30px] text-txt leading-tight">{s.value}</div>
              <div className="mt-3 font-mono text-[14px] uppercase tracking-widest leading-relaxed" style={{ color: "rgba(255,255,255,0.5)" }}>{s.label}</div>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <section className="container-x py-24">
      <div className="flex items-end justify-between mb-10">
        ...
        <Link to="/compare" className="btn-cyan !py-2 !px-3 !text-[15px] hidden sm:inline-flex">
...
function PreviewRow({ exchange, isLast, index = 0 }) {
  ...
  return (
    ...
    >
      <span className="font-mono text-[15px] text-muted w-8">#{exchange.rank}</span>
      <ExchangeLogo domain={exchange.domain} name={exchange.name} size={28} />
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className="font-semibold text-[18px]">{exchange.name}</span>
          {exchange.micarLicensed && <Badge tone="emerald">MiCAR</Badge>}
          {exchange.featured && <Badge tone="rust">Featured</Badge>}
        </div>
        <div className="text-[15px] text-muted mt-[2px]">{exchange.bestFor}</div>
      </div>
      <div className="hidden md:flex items-center gap-3 min-w-[180px]">
        ...
        <span className="font-mono text-[18px] text-txt w-7 text-right">{exchange.score}</span>
      </div>
      <a
  ...
  return (
    <section className="container-x py-24">
      <div className="mb-8">
        ...
        <p className="mt-4 text-[19px] max-w-2xl leading-relaxed" style={{ color: "rgba(255,255,255,0.6)" }}>
      ...
      >
        <div className="p-6 space-y-3">
          {TEASER_SIGNALS.map((s) => (
            <div key={s.text} className="font-mono text-[15px] sm:text-[16px]" style={{ color: s.color }}>
              {s.text}
            </div>
          ))}
          <div className="font-mono text-[15px] sm:text-[16px]" style={{ color: "rgba(255,255,255,0.22)" }}>
            ● ████████ · ████████ · ██████████████ · ██:██ UTC
          </div>
          <div className="font-mono text-[15px] sm:text-[16px]" style={{ color: "rgba(255,255,255,0.12)" }}>
            ● ██████ · ████████ · ████████████ · ██:██ UTC
          </div>
        </div>
        <div
...
function ProductCard({ glow, glowHover, border, wordmark, badge, badgeColor, heading, body, features, featureColor, cta }) {
  ...
  return (
    ...
    >
      ...
      <span
        className="font-mono text-[13px] uppercase tracking-widest px-2 py-[3px]"
      ...
      <p className="mt-2 text-[23px] font-semibold tracking-tight text-txt">{heading}</p>
      <p className="mt-3 text-[18px] leading-relaxed max-w-md" style={{ color: "rgba(255,255,255,0.6)" }}>{body}</p>
      <ul className="mt-6 space-y-2">
        {features.map((f) => (
          <li key={f} className="flex items-start gap-2 text-[16px] text-txt/90">
      ...
      >
        {SERVICES.map(({ icon: Icon, title, desc }) => (
          ...
          >
            ...
            <p className="mt-2 text-[18px] leading-relaxed" style={{ color: "rgba(255,255,255,0.6)" }}>{desc}</p>
            <Link
