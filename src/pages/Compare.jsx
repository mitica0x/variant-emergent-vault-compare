...
function ComparisonHero() {
  return (
    <section id="overview" className="max-w-3xl scroll-mt-20">
      ...
      <p className="mt-5 text-[20px] text-muted leading-relaxed max-w-2xl">
    ...
    >
      {TRUST_ITEMS.map(({ icon: Icon, title, desc }) => (
        ...
        >
          ...
          <div className="mt-3 font-mono text-[14px] uppercase tracking-widest text-txt">{title}</div>
          <div className="mt-1 text-[15px] text-muted">{desc}</div>
        </motion.div>
      ))}
    </motion.section>
  );
      ...
      {TABS.map((t) => {
        ...
        return (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            className={`px-4 py-2 font-mono text-[14px] uppercase tracking-widest transition-colors ${
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-baseline gap-3">
        <span className="font-mono text-[18px]" style={{ color: "rgba(255,255,255,0.6)" }}>#{exchange.rank}</span>
        <ExchangeLogo domain={exchange.domain} name={exchange.name} size={48} />
        <div>
          <div className="text-[25px] font-semibold leading-none">{exchange.name}</div>
          <div className="text-[15px] mt-1" style={{ color: "rgba(255,255,255,0.6)" }}>{exchange.bestFor}</div>
        </div>
      </div>
      ...
      <p className="mt-5 text-[16px] leading-relaxed" style={{ color: "rgba(255,255,255,0.6)" }}>
        {exchange.proSummary}
      </p>
  ...
  return (
    <div className="h-full flex flex-col -ml-3">
      <div className="flex flex-col" style={{ gap: "20px" }}>
        ...
        return (
          <div key={p.label} className="flex items-center gap-3" style={{ minHeight: "36px", display: "flex", alignItems: "center" }}>
            <span className="font-mono text-[13px] uppercase tracking-widest w-28" style={{ color: "rgba(255,255,255,0.6)" }}>
              {p.label}
            </span>
            <div className="flex-1">
              <MiniBar value={value} color={BAR_GRADIENT} delay={idx * 0.06} shown={shown} />
            </div>
            <span className="font-mono text-[14px] w-6 text-right" style={{ color: "rgba(255,255,255,0.6)" }}>{value}</span>
          </div>
        );
        })}
      </div>
      <div className="flex-1 flex flex-col items-center justify-center gap-2" style={{ marginTop: 40 }}>
        ...
        <p className="text-[13px] font-mono text-center" style={{ color: "rgba(255,255,255,0.6)" }}>
...
function CompactVital({ label, value, delta }) {
  ...
  return (
    <div className="hairline-b pb-2">
      ...
      {hasDelta && (
        <div className={`font-mono text-[13px] ${positive ? "text-emerald" : "text-rust"}`}>
...
function RadarBreakdown({ exchange, shown }) {
  ...
  return (
    ...
    >
      <div style={{ position: "absolute", top: 0, right: 0, display: "flex", alignItems: "center", gap: 6, fontFamily: "monospace", fontSize: 11, letterSpacing: "0.14em", color: "#0dbe82", textTransform: "uppercase", zIndex: 2 }}>
      ...
      >
        <ResponsiveContainer width="100%" height="100%">
          <RadarChart data={data} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
            ...
            <PolarAngleAxis
              dataKey="axis"
              tick={{ fill: "rgba(255,255,255,0.45)", fontSize: 13, fontFamily: "monospace" }}
            />
            <Radar
              name="score"
              dataKey="value"
...
function ShowMoreButton({ expanded, onToggle, collapsedLabel }) {
  ...
  return (
    <button
      ...
      className="w-full mt-3 py-3 font-mono uppercase tracking-widest text-[15px]"
      style={{
...
function RankedRow({ exchange, isLast, selectedId, setSelectedId }) {
  ...
  return (
    ...
    >
      <span className="font-mono text-[15px] text-muted w-7">#{exchange.rank}</span>
      <ExchangeLogo domain={exchange.domain} name={exchange.name} size={24} />
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className="text-[18px] font-semibold">{exchange.name}</span>
      ...
      <a
        ...
        className="btn-cyan !py-1 !px-2 !text-[14px]"
      >
  ...
  return (
    <div className="h-full flex flex-col -ml-3">
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-2" style={{ marginTop: 40 }}>
        ...
        <p className="font-mono text-[13px] text-center text-muted">· Score independent</p>
      </div>
    </div>
  );
}
  ...
  return (
    <section id="comparison" className="mt-20 scroll-mt-20">
      ...
      <div className="mt-6 overflow-x-auto hairline" style={{ borderRadius: 3 }}>
        <table className="w-full min-w-[920px] text-[16px] table-fixed">
          <TableCols />
          <ComparisonTableHead />
          <tbody>
            {head.map((e, i) => (
...
function CompareButton({ inCompare, disabled, onClick }) {
  ...
  return (
    <button
      ...
      onMouseEnter={() => setHover(true)}
      onMouseLeave={() => setHover(false)}
      className={`font-mono text-[13px] uppercase tracking-widest px-2 py-1 whitespace-nowrap ${
        emerald ? "" : "text-muted"
      ...
      <div className="flex items-center flex-wrap gap-3">
        ...
        <div className="flex items-center flex-wrap gap-2 flex-1">
          {selected.map((e) => (
            <span
              key={e.id}
              className="inline-flex items-center gap-2 font-mono text-[14px] px-3 py-1"
              style={{
                background: "#0f1422",
          ...
          {full ? (
            <span className="font-mono text-[14px] text-muted">Maximum 7 exchanges</span>
          ) : (
            <div className="relative">
              <input
                ...
                className="font-mono text-[15px] text-txt placeholder:text-muted px-3 py-1 min-w-[160px] outline-none"
                style={{
                  background: "#0f1422",
                  ...
                  >
                    {results.map((e) => (
                      ...
                      >
                        ...
                        <span className="text-[15px] font-semibold flex-1 truncate">{e.name}</span>
        ...
        <button
          type="button"
          onClick={onClear}
          className="font-mono text-[13px] uppercase tracking-widest text-muted hover:text-rust transition-colors"
        >
          Clear all
        </button>
      </div>
...
function ComparisonRow({ exchange, isLast, inCompare = false, compareFull = false, onToggleCompare }) {
  return (
    <tr className={`${isLast ? "" : "hairline-b"} hover:bg-white/[0.02] transition-colors`}>
      ...
      <td className="p-3 font-mono text-[15px] text-muted">{exchange.porCadence}</td>
      <td className="p-3 font-mono text-[15px] text-muted">
...
function ScorePill({ score }) {
  return (
    <span
      className={`px-2 py-[2px] font-mono text-[15px] ${scoreColor(score)}`}
  ...
  return (
    ...
    >
      <div>
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="font-mono text-[14px] text-muted">#{e.rank}</span>
        ...
        <p className="mt-5 text-[18px] text-txt/90 leading-relaxed">{e.proSummary}</p>
        <div className="mt-6">
...
function MethodologySection() {
  return (
    <section id="methodology" className="mt-20 scroll-mt-20">
      ...
      <p className="mt-4 text-[18px] text-muted leading-relaxed max-w-3xl">
...
function IndependenceNote() {
  return (
    <div className="mt-10 p-7 hairline" style={{ borderRadius: 3, background: "#0f1422" }}>
      ...
      <p className="mt-3 text-[18px] text-muted leading-relaxed">
      ...
      <Link to="/about" className="btn-cyan mt-6 !text-[15px]">
