        ...
        <Link to={`/news/${featured.slug}`} className="block mt-20">
          <div className="hairline p-8 lg:p-12 card-lift" style={{ borderRadius: 3, background: "#0f1422" }}>
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="font-mono text-[11px] text-muted">{featured.date} · {featured.readMin} min read</span>
            </div>
            <h2 className="mt-5 text-[28px] lg:text-[40px] font-bold tracking-tight max-w-3xl leading-tight">
              {featured.title}
            </h2>
            <p className="mt-4 text-[15px] text-muted max-w-3xl leading-relaxed">{featured.excerpt}</p>
            <div className="mt-6 flex items-center gap-3">
              <span className="text-[13px] text-txt/90">By {featured.author}</span>
      ...
      <section className="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px" style={{ background: "rgba(255,255,255,0.07)" }}>
        {grid.map((a) => (
          <Link key={a.slug} to={`/news/${a.slug}`} className="bg-bg p-6 card-lift block" style={{ border: "0.5px solid transparent" }}>
            ...
            <p className="mt-3 text-[13px] text-muted line-clamp-3">{a.excerpt}</p>
            <div className="mt-5 flex items-center justify-between">
              <span className="font-mono text-[11px] text-muted">{a.date}</span>
              <span className="font-mono text-[11px] text-muted">{a.readMin} min</span>
            </div>
          </Link>
        ))}
      </section>
      ...
      <section className="mt-20 hairline p-10" style={{ borderRadius: 3, background: "#0f1422" }}>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-center">
          <div>
            ...
            <p className="mt-3 text-[14px] text-muted max-w-md">Score deltas, license changes and the moves that matter. One email, every Friday.</p>
          </div>
          <form onSubmit={(e) => e.preventDefault()} className="flex gap-3">
            <div className="flex-1 flex items-center gap-2 input-base">
              <Mail size={14} className="text-muted" />
              <input className="bg-transparent flex-1 outline-none text-[14px]" placeholder="you@operator.com" />
            </div>
