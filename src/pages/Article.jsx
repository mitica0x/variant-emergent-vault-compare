...
function ArticleHeader({ article }) {
  return (
    <>
      ...
      <p className="mt-6 text-[20px] text-muted leading-relaxed max-w-2xl">{article.excerpt}</p>
    </>
  );
}
    ...
    >
      {HERO_STATS.map((s) => (
        <div key={s.label}>
          ...
          <div className={`font-mono text-[26px] mt-1 ${s.tone}`}>{s.value}</div>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="hairline p-6 mt-4" style={{ borderRadius: 3 }}>
      <div className="flex items-center gap-4">
        <span className="font-mono text-[20px] text-emerald">#{rank}</span>
        <ExchangeLogo domain={e.domain} name={e.name} size={40} />
        <div className="flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="font-semibold text-[20px]">{e.name}</span>
      ...
      <p className="mt-4 text-[15px] text-muted leading-relaxed">{e.proSummary}</p>
      <a
        href={e.affiliateUrl}
        target="_blank"
        rel="noopener noreferrer"
        className="btn-cyan mt-4 !text-[13px] !py-2"
      >
...
function InlineCTA() {
  return (
    <div className="mt-12 hairline p-7" style={{ borderRadius: 3, background: "#0f1422" }}>
      ...
      <p className="mt-2 text-[15px] text-muted">22 venues, scored on seven pillars, every 30 days.</p>
          ...
          <nav className="mt-3 flex flex-col">
            {TOC.map((t) => (
              <a
                key={t.id}
                href={`#${t.id}`}
                className="py-2 text-[14px] text-muted hover:text-txt transition-colors hairline-b"
              >
                {t.label}
              </a>
            ))}
...
function SidebarNewsletter() {
  return (
    <div className="hairline p-5" style={{ borderRadius: 3 }}>
      ...
      <p className="mt-1 text-[13px] text-muted">One email. Friday.</p>
