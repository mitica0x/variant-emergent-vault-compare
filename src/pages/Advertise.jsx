    ...
    <section className="hairline-b">
      <div className="container-x pt-12 md:pt-[153px] pb-20 grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-12 items-end">
        <div>
          ...
          <p className="mt-20 font-mono text-[13px] uppercase tracking-widest text-muted">
            Faster:{" "}
    ...
    >
      {HERO_METRICS.map((s) => (
        <div key={s.l} className="bg-bg p-5">
          <div className="font-mono text-[28px] text-txt">{s.v}</div>
      ...
      <div className="container-x py-10 grid grid-cols-1 md:grid-cols-4 gap-8">
        {TRUST_ITEMS.map((s) => (
          <div key={s.t}>
            ...
            <div className="mt-3 font-semibold text-[16px]">{s.t}</div>
            <div className="mt-1 text-[15px] text-muted">{s.d}</div>
          </div>
        ))}
      </div>
    </section>
      ...
      >
        {WHY_CARDS.map((c) => (
          ...
          >
            ...
            <p className="mt-2 text-[16px] text-muted leading-relaxed">{c.d}</p>
          </div>
        ))}
      </div>
    </section>
...
function BundleCard({ bundle: b }) {
  return (
    <div className="bg-bg p-7 card-lift" style={{ border: "0.5px solid transparent" }}>
      ...
      <p className="mt-3 text-[15px] text-muted leading-relaxed">{b.desc}</p>
      <div className="mt-5">
        <div className="font-mono text-[12px] uppercase tracking-widest text-muted">Bundle includes</div>
        <ul className="mt-2 space-y-1">
          {b.includes.map((x) => (
            <li key={x} className="flex items-start gap-2 text-[15px] text-txt/90">
      ...
      <a href="#contact" className="btn-cyan mt-6 !text-[14px]">
...
function PlacementsGrid() {
  return (
    <div className="mt-16">
      ...
      <p className="mt-6 font-mono text-[13px] uppercase tracking-widest text-muted">
...
function PlacementCard({ placement: s }) {
  return (
    <div className="bg-bg p-7 card-lift" style={{ border: "0.5px solid transparent" }}>
      <div className="flex items-center justify-between">
        <span className="font-mono text-[13px] text-muted">{s.i}</span>
      ...
      <div className="mt-5 font-mono text-[12px] uppercase tracking-widest text-muted">
        Placement · {s.page}
      </div>
      <a
        href="#contact"
      ...
      <div className="container-x py-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          <div className="text-[17px] text-muted leading-relaxed space-y-4">
            ...
            <p className="font-mono text-[14px] uppercase tracking-widest text-emerald">
              All Signal. 0 Guess.
            </p>
          </div>
        </div>
...
function LogoStrip({ names }) {
  return (
    <div className="mt-6 flex flex-wrap gap-x-10 gap-y-3">
      {names.map((n) => (
        <span key={n} className="font-mono text-[17px] text-muted hover:text-txt transition-colors">
          {n}
        </span>
      ))}
    </div>
...
function ContactInfo() {
  return (
    <div>
      ...
      <p className="mt-4 text-[16px] text-muted leading-relaxed">We respond within 24 hours.</p>
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <Field label="Message" icon={MessageSquare}>
            <textarea
              ...
              className="bg-transparent outline-none w-full text-[16px] resize-none"
          ...
          <p className="font-mono text-[12px] uppercase tracking-widest text-muted text-center">
  ...
  return (
    <div className="flex items-start gap-3">
      ...
      <div>
        ...
        <p className="mt-2 text-[16px] text-muted">We'll respond within 24 hours.</p>
      </div>
    </div>
  );
}
...
function InterestPicker({ selected, onToggle }) {
  return (
    <div>
      <div className="font-mono text-[12px] uppercase tracking-widest text-muted mb-2">
        Interested in
      </div>
...
function Field({ label, icon: Icon, children }) {
  return (
    <label className="block">
      <div className="font-mono text-[12px] uppercase tracking-widest text-muted mb-2">{label}</div>
